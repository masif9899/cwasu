  ...
  */
  ...
  function  fm_check_addons_compatibility() {
    ...
    $add_ons = array(
      ...
      'form-maker-stripe' => array('version' => '1.2.7', 'file' => 'fm_stripe.php'),
    ...
    switch ( $event->type ) {
      case 'charge.captured':
        ...
        if ( $endpoint_object->captured ) {
          global $wpdb;
          $group_id = $wpdb->get_var($wpdb->prepare("SELECT group_id FROM " . $wpdb->prefix . "formmaker_submits WHERE element_value = '" . $event->data->object->payment_intent . "'"));
